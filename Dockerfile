FROM ubuntu:latest
WORKDIR /
RUN apt update && apt upgrade -y && apt install -y git wget curl g++ zlib1g-dev make automake autoconf patch grep bzip2 gzip unzip wget git sox gfortran libtool subversion gawk python2.7 python3 python3-pip
